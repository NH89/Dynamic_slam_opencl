...
void Dynamic_slam::getFrameData_vec( frame_datum &datum ){  // Dynamic_slam::initialize_camera_vec(),  Dynmaic_slam::nextFrame()
	...
	delete [] ch;
																																			if(verbosity>local_verbosity_threshold) {
																																				...
																																				cout << "\n\n T = \n" << T;
																																				cout << "\n\n cameraMatrix = \n" << cameraMatrix;
																																				PRINT_MATX44F(frame_data.back().frame_data.prev_pose2pose,);		// correct here
																																				cout << endl << flush;
																																			}
	...
	*/
	...
	runcl.current_frames[ runcl.current_frames_idx[0] ].pose_gt		= datum.frame_data_GT.pose;
																																			if(verbosity>local_verbosity_threshold) {
																																				...
																																				if( frame_data.size() > 1 ){
																																					PRINT_MATX44F( frame_data[ frame_data.size() -2].frame_data_GT.pose, )
																																				}
																																				PRINT_MATX44F( datum.frame_data_GT.prev_pose2pose, );		// seems correct
...
*/
																																			if(verbosity>local_verbosity_threshold) cout << "\n Dynamic_slam::set_artif_pose_error_chk Finished ######################################"<<flush;
}
