...
void Dynamic_slam::binocular_reference_frame2(){
	...
	for (int layer = runcl.mm_stop ; layer >=0 ; layer-- ){
		...
		runcl.mean_sq_cols(   layer, iter, "ref_img_mean_buf", 		"ref_img_sq_mean_buf", 		runcl.ref_img_mean_rows_buf, runcl.ref_img_sq_mean_rows_buf, runcl.ref_img_mean_buf, 		runcl.ref_img_sq_mean_buf);
	}
	// predict warp ? from previous keyframe
...
void Dynamic_slam::binocular_disparity2(){
	...
	for (int layer = runcl.mm_stop ; layer > runcl.mm_start+1 ; layer-- ){										if(verbosity>local_verbosity_threshold){ cout << "\n Dynamic_slam::disparity() chk_1 layer="<<layer<<", runcl.mm_start="<<runcl.mm_start<<flush;}
		for (int iter = 0 ; iter < 5   ; iter++ ){																if(verbosity>local_verbosity_threshold){ cout << "\n Dynamic_slam::disparity() chk_2 layer="<<layer<<", iter="<<iter<<"_"<<flush;}
			runcl.warp_image(      layer, iter  );
			runcl.mean_sq_3rows(   layer, iter, "warped_img_mean_rows_buf", "warped_img_sq_mean_rows_buf", runcl.warped_img_buf, 	 	   runcl.warped_img_mean_rows_buf, 	  runcl.warped_img_sq_mean_rows_buf);
			runcl.mean_sq_cols(    layer, iter, "warped_img_mean_buf", 		"warped_img_sq_mean_buf", 	   runcl.warped_img_mean_rows_buf, runcl.warped_img_sq_mean_rows_buf, runcl.warped_img_mean_buf, 		runcl.warped_img_sq_mean_buf);
