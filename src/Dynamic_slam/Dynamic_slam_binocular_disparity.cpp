...
void Dynamic_slam::binocular_disparity(){
	...
	// compute warp for this frame
	for (int layer = runcl.mm_stop ; layer >=runcl.mm_start ; layer-- ){										if(verbosity>local_verbosity_threshold){ cout << "\f Dynamic_slam::disparity() chk_1 layer="<<layer<<flush;}
		for (int iter = 0 ; iter < 5   ; iter++ ){																if(verbosity>local_verbosity_threshold){ cout << "\f Dynamic_slam::disparity() chk_2 layer="<<layer<<", iter="<<iter<<"_"<<flush;}
