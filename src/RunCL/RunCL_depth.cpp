	...
	_clEnqueueNDRangeKernel(										// NB depth iteration is internal to the kernel within the layer.  Regularization and propagation to next layer requires further kernels.
		...
		fname					//string           fname
																																...
																																if( verbosity>local_verbosity_threshold) {
																																	...
																																	// no longer works with current version of fn. //DownloadAndSaveDepthUpdate( layer, depth_iter_per_layer  );
																																	/*
...
void RunCL::regularize_depth(uint write_layer ){
	...
	_clSetKernelArg( kernel, 6, sizeof(int),						&dm_data_stop,/*stop_offset,*/										fname);		// __private	uint		stop_offset,				//4
