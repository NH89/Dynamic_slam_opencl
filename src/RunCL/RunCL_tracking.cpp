...
void RunCL::update_tracking_depthmap(cl_mem depthmap_){
	...
	status = clEnqueueCopyBuffer( m_queue,  depthmap_, keyframe_depth_mem, 0, 0, mm_size_bytes_C1, 0, NULL, &writeEvt);	if(status!= CL_SUCCESS){cout << " status = " << checkerror(status) <<", Error: RunCL::update_tracking_depthmap(..)_clEnqueueCopyBuffer\n" << flush;exit_(status);}
...
void RunCL::initialize_tracking_depthmap(float initial_depth){
	...
	_clEnqueueFillBuffer(uload_queue, depth_mem, 	&initial_depth, sizeof(float), 0, mm_size_bytes_C1, 	0, NULL, &writeEvt, fname);

	status = clFinish(uload_queue);																											if(status!= CL_SUCCESS){cout << " status = " << checkerror(status) <<", Error: RunCL::initialize_tracking_depthmap(..)_clfinish_clEnqueueCopyBuffer\n" << flush;exit_(status);}
...
void RunCL::se3_rho_sq(const uint local_num_samples,  const uint start_sample_idx,  float Rho_sq_results[tracking_tot_samples][max_mipmap_layers][tracking_num_colour_channels], const float count[4], uint start, uint stop,  float k2k_3_16_[tracking_tot_samples][16]   ){
	...
	status = clEnqueueWriteBuffer(uload_queue, k2kbuf,	CL_FALSE, 0, local_num_samples*16*sizeof(float), k2k_3_16_[start_sample_idx], 	0, NULL, &writeEvt);	if (status != CL_SUCCESS)	{ cout << "\nstatus = " << checkerror(status) <<"\n"<<flush; cout << "Error: RunCL::se3_rho_sq(..)_chk0.5\n" << endl;exit_(status);}	clFlush(uload_queue); status = clFinish(uload_queue);
	float zero  = 0;
	...
	//input integers
	//      __private	 uint layer, set in mipmap_call_kernel(..) below                                                                                                                              	  	__private	    uint	    layer,		                    //0
	res = clSetKernelArg(se3_rho_sq_kernel, 1, sizeof(size_t), &local_num_samples);										if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__private		uint		local_num_samples				//1
	res = clSetKernelArg(se3_rho_sq_kernel, 2, sizeof(size_t), &se3_sum_size);											if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__private		uint		pix_sum_size_bytes,				//2
	//input buffers
    res = clSetKernelArg(se3_rho_sq_kernel, 3, sizeof(cl_mem), &mipmap_buf);											if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__constant    uint*	    mipmap_params,	                //3
	res = clSetKernelArg(se3_rho_sq_kernel, 4, sizeof(cl_mem), &uint_param_buf);										if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__constant	uint*		uint_params,					//4
	res = clSetKernelArg(se3_rho_sq_kernel, 5, sizeof(cl_mem), &fp32_param_buf);										if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__constant	float*		fp32_params						//5
	res = clSetKernelArg(se3_rho_sq_kernel, 6, sizeof(cl_mem), &k2kbuf);												if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__global		float* 		k2k,							//6		// TODO keyframe2K
	res = clSetKernelArg(se3_rho_sq_kernel, 7, sizeof(cl_mem), &keyframe_imgmem);										if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__global 		float4*		keyframe_imgmem,				//7		// TODO need keyframe mipmap   keyframe_imgmem , keyframe_depth_mem
	res = clSetKernelArg(se3_rho_sq_kernel, 8, sizeof(cl_mem), &imgmem);												if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__global 		float4*		imgmem,							//8
	res = clSetKernelArg(se3_rho_sq_kernel, 9, sizeof(cl_mem), &keyframe_depth_mem);									if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__global 	 	float*		keyframe_depth_mem				//9		// NB GT_depth, now stoed as inv_depth	// TODO need keyframe mipmap
	res = clSetKernelArg(se3_rho_sq_kernel,10, sizeof(cl_mem), &keyframe_g1mem);										if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__global 	 	float8*		g1p								//10

	//output
	res = clSetKernelArg(se3_rho_sq_kernel,11, sizeof(cl_mem), &SE3_rho_map_mem);										if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__global	    float4*     Rho_					        //11
	res = clSetKernelArg(se3_rho_sq_kernel,12, (local_num_samples*local_work_size*4/wg_divisor)*sizeof(float), NULL);	if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__local		float4*		local_sum_rho_sq				//12	// 1 DoF, float4 channels
	res = clSetKernelArg(se3_rho_sq_kernel,13, sizeof(cl_mem), &se3_sum_rho_sq_mem);		 							if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__global 		float4*		global_sum_rho_sq,				//13
...
void RunCL::estimateSE3_LK(float local_k2k[16], float SE3_results[max_mipmap_layers][num_SE3_DoF][tracking_num_colour_channels], float SE3_weights_results[max_mipmap_layers][num_SE3_DoF][tracking_num_colour_channels], float Rho_sq_results[max_mipmap_layers][4], int count, uint start, uint stop){ //estimateSE3_LK(); 	(uint start=0, uint stop=8)			// TODO replace arbitrary fixed constant with a const uint variable in the header...
	...
	status = clEnqueueWriteBuffer(uload_queue, k2kbuf,	CL_FALSE, 0,  16 * sizeof(float), local_k2k, 				0, NULL, &writeEvt);	if (status != CL_SUCCESS)	{ cout << "\nstatus = " << checkerror(status) <<"\n"<<flush; cout << "Error: RunCL::estimateSE3_LK(..)_chk0.5\n" << endl;exit_(status);}	clFlush(uload_queue); status = clFinish(uload_queue);
	float zero  = 0;
	...
	status = clFlush( uload_queue );																										if (status != CL_SUCCESS)	{ cout << "\nclEnqueueWriteBuffer clFlush( uload_queue ) status = " << checkerror(status) <<"\n"<<flush; exit_(status); }
	clFinish( uload_queue );
																																			if(verbosity>local_verbosity_threshold) {cout<<"\n\nRunCL::estimateSE3_LK(..)_chk_2 "<<flush;}
																																			// NB GT_depth loaded to depth_mem by void RunCL::loadFrameData(..)
	cl_int 	res;
	const 	uint wg_divisor =2;  // 1,2,4,8  reduction in workgroup size for this kernel.
	// inputs
	...
	// 	res = clSetKernelArg(se3_lk_grad_kernel, 9, sizeof(cl_mem), &amem);		/* TODO remove to proper place */	if(res!=CL_SUCCESS){cout<<"\nres = "<<checkerror(res)<<"\n"<<flush;exit_(res);}		//__global 	 	float*		amem							//9		// NB GT_depth, now stoed as inv_depth	// TODO need keyframe mipmap
	// }else{
