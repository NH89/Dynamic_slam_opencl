...
__kernel void use_inferred_depthmap(
	...
	__private	uint		depth_width_out,			//3
	__private	uint		patch_height,				//4
	__private	uint		stop_offset,				//5

	__constant 	uint4*		lookup_table,				//6

	__global 	float2*		temp_depth,					//7
	__global	float2*		depth_map					//8
	)
{
	uint global_id_uint 					= get_global_id(0);
	uint4	lookup_ref						= lookup_table[	global_id_uint + lookup_table_read_offset];
	...
	uint	u								= lookup_ref.x;														// read_column
	uint	v								= lookup_ref.y;														// read_row
	uint read_idx							= lookup_ref.z	- depth_in_width - 1;
...
// 	uint read_idx_depth						= depth_read_offset + pixel_offset;
	...
	for (int i=1; i<3; i++){
		for (int j=0; j<3; j++){
			depth_in[i][j]					= depth_map[read_idx];
			read_idx++;
		}
		read_idx 							+= depth_in_width - 3;
	}

	int		write_index_start 				= write_offset + u*4 + (v * 4 * depth_in_width);

	for (int i=0; i<patch_height; i++){
		float offset_u 							= 3.0f/8.0f;
		float offset_v 							= 3.0f/8.0f;
		int	  write_index						= write_index_start + i*depth_in_width;
		...
		__attribute__((opencl_unroll_hint))
		for(int i=0; i<3; i++){
			depth_in[ arr_idx[2] ][i]			= depth_map[read_idx];
			read_idx++;
		}
		read_idx 								+= depth_in_width - 3;
		...
		for (int i=0; i<2; i++){
			for (int j=0; j<2; j++){
				for (int k=0; k<2; k++){
					for (int l=0; l<2; l++){
						depth_map[write_index]	= offset_v*(  offset_u*depth_in[i][j]	+	(1-offset_u)*depth_in[i][j+1] )	+ 	(1-offset_v)*(  offset_u*depth_in[i+1][j]	+	(1-offset_u)*depth_in[i+1][j+1]		);
						offset_u				-= 0.25;
						write_index				+= 1;
					}
					offset_u					+= 0.25;
					offset_v					-= 0.25;
					write_index					+= depth_width_out -1;
				}
				offset_u 							+= 0.5;
				offset_v 							+= 0.25;
				write_index							-= depth_width_out -1;
