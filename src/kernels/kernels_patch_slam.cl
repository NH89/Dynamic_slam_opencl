...
__kernel void  patch_img_grad(						// To be launched with 1 thread per col for 32x32 patches, and an integer multiple of 32 threads.
	...
	__global 	float4*		SE3_Hessian_map,		//11											// HSV (6x6) matrix so 36*float8
	__local		float4*		local_Hessian,			//12												//	local_Hessian[ sizeof(float4) *6*6 *local_size]
				...
				for (uint i=0; i<3; i++) {																																										// select only ST3
					for (uint j=0; j<3; j++) {
						...
						if( fmod((float)lid,out_block_size) == 0  ){																																			// selects columns i.e. threads within the workgroup
																						SE3_Hessian_map[	offset_1_1 ]	= Hessian_pvt_arr[	block_row ][i][j] / Hessian_pvt_arr[	block_row ][i][j].w;
...
__kernel void  patch_hessian_reduce(						// one workgroup per element.
	...
	__global 	float4*		SE3_Hessian_map					//5	// write result to one before start_idx
){
	uint				lid				= get_local_id(0);
	uint 				index			= lid + start_idx;
	float4				pvt_Hessian		= 0;								// TODO max_rows macro ?
	...
	if( fmod((float)lid, step)				==0 ) {
		uint where				=	elem;						// start_idx - mm_cols +
		SE3_Hessian_map[where]	=	pvt_Hessian;				// write result to the first 36 pixels of SE3_Hessian_map, directly above this layer of hessian pyramid,  because this will be fastest to read to CPU.
